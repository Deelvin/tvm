...
class GATuner(Tuner):
    ...
    def next_batch(self, batch_size):
        ...
        while len(ret) < batch_size and self.trial_pt < self.space.total_length:
            ...
            self.trial_pt += 1
            cfg = self.space.get(self.space.knob2point(gene))
            if cfg:
                ret.append(cfg)
        return ret
        ...
        if len(self.scores) >= len(self.genes) and len(self.visited) < self.space.filtered_length:
            ...
            for tmp_gene in tmp_genes:
                ...
                if len(self.visited) < self.space.filtered_length:
                    while self.space.knob2point(tmp_gene) in self.visited:
                        j = np.random.randint(len(self.space.dims))
                        tmp_gene[j] = np.random.randint(
                            self.space.dims[j]  # pylint: disable=invalid-sequence-index
                        )

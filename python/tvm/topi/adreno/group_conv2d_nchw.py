...
def group_conv2d_nchwc(cfg, Input, Filter, stride, padding, dilation, groups, out_dtype):
    """
    ...
    rcc = te.reduce_axis((0, in_channel_chunks // groups), name="rc")
    ...
    conv = te.compute(
        ...
        lambda nn, ffc, yy, xx, ffb: te.sum(
            (
                temp[nn, ffc // (out_channel_chunks // groups) * (in_channel_chunks // groups) + rcc, yy * stride_h + ry * dilation_h, xx * stride_w + rx * dilation_w, rcb]
                * Filter[ffc, rcc * in_channel_block + rcb, ry, rx, ffb]
            ).astype(out_dtype),
            axis=[rcc, rcb, ry, rx],
        ),
        tag="group_conv2d_nchwc",
    )
...
def schedule_group_conv2d_NCHWc_KCRSk(cfg, s, output):
    ...
    g, fc = s[latest_blocked].split(fc, nparts=groups)
    bg, vg = cfg["tile_g"].apply(s, latest_blocked, g)
    ...
    s[latest_blocked].reorder(n, bg, bf, by, bx, vg, vf, vy, vx, tf, ty, tx, fb)
    bg = s[latest_blocked].fuse(n, bg)
    bf = s[latest_blocked].fuse(bg, bf)
    ...
    s[latest_blocked].bind(vg, te.thread_axis("vthread"))

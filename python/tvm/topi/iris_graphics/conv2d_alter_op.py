...
"""Conv2D alter op for Intel Iris Graphics GPU"""
...
from ..adreno.utils import infer_tile_size
...
@conv2d_alter_layout.register("iris_graphics")
def _alter_conv2d_layout(attrs, inputs, tinfos, out_type):
    ...
    if "conv2d_nchw_winograd" in topi_tmpl:
        ...
        wkl_name = "conv2d_nchw_winograd_without_weight_transform" + suffix + ".iris"
        if dilation != (1, 1):
            logger.warning("Does not support weight pre-transform for dilated convolution.")
            return None
        ...
        if len(data_tensor.shape) == 5:
            assert (data_layout in ("NCHW4c", 'NCHW8c')) and (kernel_layout in ("OIHW4o", "OIHW8o"))
            N, CI, H, W, CB = get_const_tuple(data_tensor.shape)
            CO, _, KH, KW, COB = get_const_tuple(kernel_tensor.shape)
            weight = relay.layout_transform(inputs[1], kernel_layout, "OIHW")
            weight = relay.nn.contrib_conv2d_winograd_weight_transform(weight, tile_size=tile_size)
            weight = relay.layout_transform(weight, "HWOI", kernel_layout)
        ...
        if in_channel_block % 4 != 0 or num_filter_block % 4 != 0:
            new_workload = autotvm.task.args_to_workload(
                [new_data, new_weight, strides, padding, dilation, out_dtype],
                wkl_name,
            )
    ...
    if "conv2d_nhwc_winograd" in topi_tmpl:
        suffix = "_acc32" if "acc32" in topi_tmpl else ""
        wkl_name = "conv2d_nhwc_winograd_without_weight_transform" + suffix + ".iris"
        if dilation != (1, 1):
            logger.warning("Does not support weight pre-transform for dilated convolution.")
            return None
        ...
        tile_size = infer_tile_size(data_tensor, data_layout)
        if len(data_tensor.shape) == 5:
            assert (data_layout in ("NHWC4c", "NHWC8c")) and (kernel_layout in ("HWIO4o", "HWIO8o"))
            N, CI, H, W, CB = get_const_tuple(data_tensor.shape)
    ...
    if "conv2d_nhwc" in topi_tmpl:  # covers both conv2d_nhwcc and depthwise_conv2d_nhwcc
        if (data_layout == "NHWC" and kernel_layout == "HWIO") or (
            ...
            in_channel_block = in_channels % 8
            if in_channel_block == 0:
                in_channel_block = 8
            else:
                ...
                if in_channel_block == 0:
                    in_channel_block = 4
            num_filter_block = out_channles % 8
            if num_filter_block == 0:
                num_filter_block = 8
            else:

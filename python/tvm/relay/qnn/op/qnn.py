...
class ReqConfig(Object):
    ...
    Do not construct directly, use reqconfig instead.
    ...
    @staticmethod
    def _get_node_default_calculation_flow_type():
    ...
    _node_defaults = {
        ...
        "calculation_flow_type": _get_node_default_calculation_flow_type.__func__,
    }
    ...
    def __init__(self, handle):
        ...
        """
        super(ReqConfig, self).__init__(handle)
        self.handle = handle

    def __enter__(self):
        # pylint: disable=protected-access
        _requantize._EnterReqConfigScope(self)
        return self

    def __exit__(self, ptype, value, trace):
        _requantize._ExitReqConfigScope()

    def __setattr__(self, name, value):
        if name in ReqConfig._node_defaults:
            raise AttributeError("'%s' object cannot set attribute '%s'" % (str(type(self)), name))
        return super(ReqConfig, self).__setattr__(name, value)


def current_reqconfig():
    """Get the current requantization configuration."""
    return _requantize._GetCurrentReqConfig()


def reqconfig(**kwargs):
    """Configure the requantization behavior by setting config variables.
    ...
    """
    node_args = {
        k: v() if k not in kwargs else kwargs[k] for k, v in ReqConfig._node_defaults.items()
    }
    return tvm.ir.make_node("relay.qnn.op.ReqConfig", **node_args)

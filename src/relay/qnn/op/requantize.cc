...
Expr RequantizeLowerFP(const Expr& input_tensor, const Expr& input_scale,
  ...
  if (IsConstScalar(input_scale)) {
    ...
    double double_multiplier = static_cast<double>(input_scale_float) / double(output_scale_float);
    // Skip if input and output scales are same.
    if (!IsEqualScalar(input_scale, output_scale)) {
...
Expr RequantizeLower(const Expr& input_tensor, const Expr& input_scale,
  ...
  ICHECK(param->rounding == "UPWARD" || param->rounding == "TONEAREST")
      ...
      << "TONEAREST";
  // Check calculation flow validity.
  ICHECK(param->calculation_flow_type == "int64" || param->calculation_flow_type == "float32" ||
         param->calculation_flow_type == "float64")
      << "QNN requantize supports three calculation flow types - \"int64\", \"float32\" and "
         "\"float64\"";
  if (param->calculation_flow_type == "float32")
  ...
  else if (param->calculation_flow_type == "float64")
    return RequantizeLowerFP<64>(input_tensor, input_scale, input_zero_point, output_scale,
                                 output_zero_point, param, input_shape, out_dtype);
  else /*if (param->calculation_flow_type == "int64") */
...
Expr RequantizeQnnCanonicalize(const Attrs& attrs, const Array<Expr>& new_args,
  ...
  const ReqConfig& cfg = ReqConfig::Current();
  ...
  const_cast<RequantizeAttrs*>(param)->calculation_flow_type =
      SelectRequntizeParameter(param->calculation_flow_type, cfg->get_calculation_flow_type(),
                               cfg->is_default, "calculation_flow_type");
...
Expr MakeRequantize(Expr data, Expr input_scale, Expr input_zero_point, Expr output_scale,
                    Expr output_zero_point, int axis, String rounding, String calculation_flow_type,
                    DataType out_dtype) {
  auto attrs = make_object<RequantizeAttrs>();
  attrs->axis = axis;
  attrs->rounding = std::move(rounding);
  attrs->out_dtype = std::move(out_dtype);
  attrs->calculation_flow_type = std::move(calculation_flow_type);

...
inline static const std::string get_ref_layout(const std::string& src) {
  ...
  if (iter == layout_map.end()) {
    LOG(FATAL) << "Can not find reference memory layout for conv: " << src;
    return "";
  }
  return iter->second;
}
...
inline static std::vector<int> permutation(const std::string& src) {
  const std::string ref = get_ref_layout(src);
...
inline static dnnl::memory::dims reshape(const dnnl::memory::dims dims,
  ...
  for (auto c : axis) {
    auto pos = layout.find(c);
    if (pos != std::string::npos) {
      shape.push_back(dims[pos]);
      auto lower_c = std::tolower(c);
      auto lower_pos = layout.find(lower_c);
      if (lower_pos != std::string::npos) {
        shape.back() *= dims[lower_pos];
      }
    }
  }
  return shape;
...
class TensorRequisite {
 public:
  ...
  TensorRequisite layout(dnnl::memory::format_tag tag) const {
    auto orig = std::make_shared<TensorRequisite>(*this);
    // Recreate tensor desc with layout 'any'
    dnnl::memory::desc any_desc{t_desc_.dims(), t_desc_.data_type(), tag};
...
class NodeHelper {
 public:
  NodeHelper(const size_t& nid, GraphExplorer &graph_explorer)
  ...
  std::string GetOpName() const { return node_.GetOpName(); }

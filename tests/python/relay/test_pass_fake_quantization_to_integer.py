...
def test_dequantize_propagation_1():
    ...
    op0 = relay.qnn.op.qnn.batch_matmul(
        a, b, relay.const(0), relay.const(0), relay.const(1.5), relay.const(0.5)
    )
...
def test_dequantize_propagation_11():
    # negative test prop before bad op
    shape_x = [1, 4, 2]
    shape_w = [1, 4, 2]
    a = relay.var("a", shape=shape_x, dtype="int8")
    b = relay.var("b", shape=shape_w, dtype="int8")
...
def test_dequantize_propagation_2():
    ...
    expected_expr = relay.qnn.op.qnn.dequantize(op0, relay.const(0.75), relay.const(0))
...
def test_dequantize_propagation_3():
    ...
    op1 = relay.qnn.op.quantize(
        relay.const(2.0), relay.const(0.75), relay.const(0), out_dtype="int32"
    )
    ...
    expected_expr = relay.qnn.op.requantize(
        op2, relay.const(0.75), relay.const(0), relay.const(1.0), relay.const(0), out_dtype="int8"
    )
...
def test_dequantize_propagation_4():
    ...
    op3 = relay.qnn.op.qnn.batch_matmul(
        op0, op1, relay.const(0), relay.const(0), relay.const(1.5), relay.const(0.5)
    )
...
def test_dequantize_propagation_5():
    # prop single no calc scale
    shape_x = [1, 2, 4]
    a = relay.var("a", shape=shape_x, dtype="int8")
    ...
    op0 = relay.op.reshape(a, (1, 4, 2))
    op1 = relay.qnn.op.dequantize(op0, relay.const(1.5), relay.const(0))

...
calculation_flow_types = ["float32", "float64", "int64"]
...
def get_mod(
    ...
    mod = relay.qnn.op.requantize(
        ...
        axis=axis,
        rounding=rounding,
        calculation_flow_type=calculation_flow_type,
        out_dtype=out_dtype,
    )
...
def test_same_scale():
    ...
    for calculation_flow_type in calculation_flow_types:
...
def test_non_default_cfg_and_no_args():
    for rounding_cfg in roundings:
        with relay.qnn.op.reqconfig(rounding=rounding_cfg):
            mod = get_mod(
                data_shape=(32,),
                data_dtype="int32",
                out_dtype="int8",
...
def test_default_cfg_and_args():
    for rounding in roundings:
        with relay.qnn.op.reqconfig(rounding="UPWARD"):
            mod = get_mod(
                data_shape=(32,),
                data_dtype="int32",
                out_dtype="int8",
